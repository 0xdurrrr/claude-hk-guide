...
1. 在 App Store 或 Google Play 搜尋「**Avalanche Card**」並下載安裝。
2. 開啟 App，使用電郵註冊帳號。
3. 按指引完成 **KYC 身份驗證**：
...
1. 在 App Store 或 Google Play 搜尋「**Kraken**」並下載安裝。
2. 註冊帳號並登入。
...
- **保管好助記詞與密碼：** Avalanche Card 和 Kraken 帳號一旦遺失無法找回。
- **啟用雙重驗證（2FA）：** 建議使用 Google Authenticator 而非簡訊驗證。
